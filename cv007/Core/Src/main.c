...
void StartVisualTask(void const * argument)
{
	...
	for(;;)
	{
		if (xQueueReceive(xVisualQueueHandle, &msg, portMAX_DELAY)) { //decide by X axis tilt, leave OFF interval around horizontal position
			if (msg < -1000) {
...
void StartAcceleroTask(void const * argument)
{
	...
	printf("LIS2DW12_ID %s\n", (whoamI == LIS2DW12_ID) ? "OK" : "FAIL"); // UART output, check if device responds
	/* Infinite loop */
	for(;;)
	{
		...
		xQueueSend(xVisualQueueHandle, &raw_acceleration[0], 0); // LED output, XYZ tilt values, every 50 ms
		osDelay(50);
		numCycles++;
		if(numCycles == 20){
			printf("X=%d Y=%d Z=%d\n", raw_acceleration[0], raw_acceleration[1], raw_acceleration[2]); // UART output, XYZ tilt values, every 1000 ms
			numCycles = 0;
		}
	}
